...
for file_path in filenames:
    ...
    sorted_result_forecast = sorted_result_forecast.interpolate(method='linear', inplace=True)

    forecasted_financial_data = pd.DataFrame(columns=['Year'] + sorted_result_forecast.columns[1:].tolist())
    for col in sorted_result_forecast.columns:
        last_yr_num = sorted_result_forecast.index.year
        last_year = sorted_result_forecast[:,last_yr_num].max()
        forecast_horizon = 2  # Forecasting for the next 2 years
        financial_ratios_to_forecast = plotting_data
        historical_data = sorted_result_forecast[col]

        model = SimpleExpSmoothing(historical_data[col]).fit()
        forecast_values = model.forecast(forecast_horizon)
        forecasted_financial_data[({file_name_without_extension}, col)] = forecast_values

        forecasted_years = [last_year + i for i in range(1, forecast_horizon + 1)]
        forecasted_financial_data['Year'] = forecasted_years

        # Set the 'Year' column as the index
        forecasted_financial_data.set_index('Year', inplace=True)

        # Print the forecasted financial data
        print("Forecasted Financial Data:")
        print(forecasted_financial_data)

    print("Forecasted values for 2024 and 2025:")
    print(forecasted_financial_data)
    df_forecasted_data = pd.DataFrame(forecasted_financial_data)
